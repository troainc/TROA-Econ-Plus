...
- [x] Stage 10 - Price alerts and standing limit orders (`v1.2.4-alpha`).
...
- [x] Star-Citizen economy loops: location-based commodity pricing (trade routes/hauling), ship insurance (premiums + loss claims via the Hangar+ API), a mission/contract board (escrowed rewards), and contraband/customs enforcement; delivered with Econ+ API v2.1 (territory, location-priced market, insurance, and contract interfaces) for Hangar+.
- [x] LCD overhaul: a view-model + dual-renderer system (pretty sprite graphics with an automatic text fallback, player-selectable per panel) and new panels for the full feature set (Dashboard, Tax/jurisdiction, Contracts, Route, Insurance, Bonds, Help) plus reworked market/exchange/government/bank/faction/loan panels.
...
- [ ] Validate the LCD overhaul on a live server: name panels `[ECON+]`, verify each template in both `Render=Sprite` and `Render=Text`, auto-fit on square and wide panels, the sprite->text fallback path, a `Tax` panel resolving the jurisdiction it sits in, and `Style=false` manual control.
- [ ] Validate the Star-Citizen loops on a live server: a two-territory price gap yields a profitable haul; buy an insurance policy and confirm a bounded claim payout via the Hangar+ claim hook; post/accept/complete and cancel/expire a contract and confirm escrow capture/refund; flag a contraband symbol and confirm the customs fine (and block) on an in-territory trade; confirm the Hangar+ v2.1 API (territory, location quotes, insurance, contracts) end to end and that v2.0 consumers still work; `!econadmin boundarytest` still passes.
